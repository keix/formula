//! Headless regression for Blargg's `03-trigger.gb`.
...
#[test]
fn dmg_sound_03_trigger_reaches_pass_result() {
    let rom_path = Path::new("test-rom/gb-test-roms/dmg_sound/rom_singles/03-trigger.gb");
    if !rom_path.exists() {
        eprintln!(
            "skipping dmg_sound 03-trigger regression: drop the ROM at {} to enable it",
            rom_path.display()
        );
        return;
    }

    let rom = std::fs::read(rom_path).expect("read 03-trigger.gb");
    let mut mmu = Mmu::new(load_cartridge(rom));
    let mut cpu = post_boot_cpu();
    mmu.write8(0xff40, 0x91);
    mmu.write8(0xff47, 0xfc);

    for _ in 0..12_000_000usize {
        cpu.step(&mut mmu);
        let signature_ready =
