//! Headless regression for Blargg's `02-len ctr.gb`.
...
#[test]
fn dmg_sound_02_len_ctr_reaches_pass_result() {
    let rom_path = Path::new("test-rom/gb-test-roms/dmg_sound/rom_singles/02-len ctr.gb");
    if !rom_path.exists() {
        eprintln!(
            "skipping dmg_sound 02-len ctr regression: drop the ROM at {} to enable it",
            rom_path.display()
        );
        return;
    }

    let rom = std::fs::read(rom_path).expect("read 02-len ctr.gb");
