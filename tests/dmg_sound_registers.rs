//! Headless regression for Blargg's `01-registers.gb`.
//!
//! This locks the CPU-visible APU register semantics before timing and
//! channel state exist: read masks, NR52 power behavior, and wave RAM.
...
#[test]
fn dmg_sound_01_registers_reaches_pass_result() {
    let rom_path = Path::new("test-rom/gb-test-roms/dmg_sound/rom_singles/01-registers.gb");
    if !rom_path.exists() {
        eprintln!(
            "skipping dmg_sound 01-registers regression: drop the ROM at {} to enable it",
            rom_path.display()
        );
        return;
    }

    let rom = std::fs::read(rom_path).expect("read 01-registers.gb");
    let mut mmu = Mmu::new(load_cartridge(rom));
    let mut cpu = post_boot_cpu();

    // Match the runner's post-boot LCD palette so the test ROM's text init
    // runs under the same environment as manual execution.
