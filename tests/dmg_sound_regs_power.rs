//! Headless regression for Blargg's `11-regs after power.gb`.
...
#[test]
fn dmg_sound_11_regs_after_power_reaches_pass_result() {
    let rom_path = Path::new("test-rom/gb-test-roms/dmg_sound/rom_singles/11-regs after power.gb");
    if !rom_path.exists() {
        eprintln!(
            "skipping dmg_sound 11-regs after power regression: drop the ROM at {} to enable it",
            rom_path.display()
        );
        return;
    }

    let rom = std::fs::read(rom_path).expect("read 11-regs after power.gb");
