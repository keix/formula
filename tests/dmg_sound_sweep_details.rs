//! Headless regression for Blargg's `05-sweep details.gb`.
...
#[test]
fn dmg_sound_05_sweep_details_reaches_pass_result() {
    let rom_path = Path::new("test-rom/gb-test-roms/dmg_sound/rom_singles/05-sweep details.gb");
    if !rom_path.exists() {
        eprintln!(
            "skipping dmg_sound 05-sweep details regression: drop the ROM at {} to enable it",
            rom_path.display()
        );
        return;
    }

    let rom = std::fs::read(rom_path).expect("read 05-sweep details.gb");
    let mut mmu = Mmu::new(load_cartridge(rom));
    let mut cpu = post_boot_cpu();
    mmu.write8(0xff40, 0x91);
    mmu.write8(0xff47, 0xfc);

    for _ in 0..16_000_000usize {
        cpu.step(&mut mmu);
        let signature_ready =
