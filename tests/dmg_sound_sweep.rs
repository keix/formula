//! Headless regression for Blargg's `04-sweep.gb`.
...
#[test]
fn dmg_sound_04_sweep_reaches_pass_result() {
    let rom_path = Path::new("test-rom/gb-test-roms/dmg_sound/rom_singles/04-sweep.gb");
    if !rom_path.exists() {
        eprintln!(
            "skipping dmg_sound 04-sweep regression: drop the ROM at {} to enable it",
            rom_path.display()
        );
        return;
    }

    let rom = std::fs::read(rom_path).expect("read 04-sweep.gb");
